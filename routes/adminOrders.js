...
router.get("/", auth, admin, async (req, res) => {
    try {
        ...
        const sort =
            String(req.query.sort || "desc").toLowerCase() ===
            "asc"
                ? "ASC"
                : "DESC";
        ...
        */
        ...
        if (status) {
            ...
            conditions.push(
                `LOWER(TRIM(o.status)) = LOWER(TRIM($${values.length}))`
            );
        }
        ...
        */
        ...
        if (pricingStatus) {
            ...
            conditions.push(
                `LOWER(TRIM(o.pricing_status)) = LOWER(TRIM($${values.length}))`
            );
        }
        ...
        try {
            const result = await pool.query(`
                SELECT
                    ...
                    COUNT(*) FILTER (
                        WHERE LOWER(TRIM(pricing_status))
                            = 'pending_review'
                    )::INTEGER AS pricing_review_orders,
